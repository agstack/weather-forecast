...
###############################################
varDict = {
    fixVarName(var2): "Air Temp [C] (2 m above surface)",
...
def fixToLocalTime(df, lat, lon):
    ...
    df["FORECAST_DATE_LOCAL"] = df.FORECAST_DATE_UTC.apply(
        lambda x: x + timezone.utcoffset(x)
    )
...
def weatherForecastVariables():
    try:
        ...
        error_res[
            "db function call error"
        ] = "function call failed for getWeatherForecastVars"
...
def weatherForecast():
    ...
    try:
        ...
        error_res[
            "db function call error"
        ] = "DB function call failed for getWeatherForecast"
